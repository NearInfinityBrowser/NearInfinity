...
final class SpawnPoint extends AbstractStruct implements AddRemovable
{
  ...
  protected int read(byte buffer[], int offset) throws Exception
  {
    ...
    list.add(new DecNumber(buffer, offset + 32, 2, "Spawn location: X"));
    list.add(new DecNumber(buffer, offset + 34, 2, "Spawn location: Y"));
    for (int i = 0; i < 10; i++)
      list.add(new SpawnResourceRef(buffer, offset + 36 + (i << 3), "Creature " + (i + 1)));
    list.add(new DecNumber(buffer, offset + 116, 2, "# creatures"));
    list.add(new HexNumber(buffer, offset + 118, 2, "Spawn frequency?"));
    list.add(new DecNumber(buffer, offset + 120, 2, "Seconds between spawning"));
    list.add(new HexNumber(buffer, offset + 122, 2, "Spawn method?"));
    list.add(new Unknown(buffer, offset + 124, 4));
