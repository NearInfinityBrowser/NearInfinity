...
public class BCSTokenMaker extends AbstractTokenMaker {
  ...
  private static final String CHAR_HEX_PREFIX     = "xX";
  private static final String CHAR_BIN_PREFIX     = "bB";
  private static final String CHAR_STRING_DELIM   = "\"~%";
  ...
  public boolean getMarkOccurrencesOfTokenType(int type) {
    return type == TOKEN_ACTION || type == TOKEN_TRIGGER || type == TOKEN_OBJECT || type == TOKEN_SYMBOL
        || type == TOKEN_SYMBOL_SPELL;
  }
  ...
  public Token getTokenList(Segment text, int initialTokenType, int startOffset) {
    ...
    for (int i = ofs; i < end; i++) {
      ...
      switch (currentTokenType) {
        case TokenTypes.NULL: {
          ...
          if (c == '#') {
          ...
          } else if (CHAR_STRING_DELIM.indexOf(c) > -1) {
            // string
            stringDelimiter = c;
            currentTokenType = TOKEN_STRING;
          } else if (CHAR_WHITE_SPACE.indexOf(c) > -1) {
            // whitespace
            currentTokenType = TOKEN_WHITESPACE;
          } else if (CHAR_OPERATOR.indexOf(c) > -1) {
            // operator
            ...
            if (c == '-') {
              if (i + 1 < end && RSyntaxUtilities.isDigit(array[i + 1])) {
                ...
                if (array[i + 1] == '0' && i + 2 < end && CHAR_HEX_PREFIX.indexOf(array[i + 2]) > -1) {
                  // hex number
                  currentTokenType = TOKEN_HEXNUMBER;
                } else if (array[i + 1] == '0' && i + 2 < end && CHAR_BIN_PREFIX.indexOf(array[i + 2]) > -1) {
                  // bin number
              ...
              currentTokenType = TOKEN_NUMBER;
              if (c == '0' && i + 1 < end && CHAR_HEX_PREFIX.indexOf(array[i + 1]) > -1) {
                // hex number
                currentTokenType = TOKEN_HEXNUMBER;
              } else if (c == '0' && i + 1 < end && CHAR_BIN_PREFIX.indexOf(array[i + 1]) > -1) {
                // bin number
                currentTokenType = TOKEN_BINNUMBER;
              }
            } else {
        ...
        case TOKEN_NUMBER: {
          if (c == '#') {
            ...
            currentTokenStart = i;
            currentTokenType = TOKEN_OPERATOR;
          } else if (currentTokenType == TOKEN_HEXNUMBER
              && (RSyntaxUtilities.isHexCharacter(c) || CHAR_HEX_PREFIX.indexOf(c) > -1)) {
            // still a hex number?
            if (CHAR_HEX_PREFIX.indexOf(c) > -1 && (i == currentTokenStart || array[i - 1] != '0')) {
              currentTokenType = TOKEN_IDENTIFIER;
            }
          } else if (currentTokenType == TOKEN_BINNUMBER && (("01".indexOf(c) > -1) || CHAR_BIN_PREFIX.indexOf(c) > -1)) {
            // still a bin number?
            if (CHAR_BIN_PREFIX.indexOf(c) > -1 && (i == currentTokenStart || array[i - 1] != '0')) {
              currentTokenType = TOKEN_IDENTIFIER;
            }
    ...
    switch (currentTokenType) {
      case TokenTypes.NULL: // do nothing if no token is active
        addNullToken();
        break;
    ...
    for (final Signatures sig : signatures) {
      if (sig != null) {
        for (final Integer code : sig.getFunctionIds()) {
          Signatures.Function[] functions = sig.getFunction(code);
