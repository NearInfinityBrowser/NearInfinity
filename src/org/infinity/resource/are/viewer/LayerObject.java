...
public abstract class LayerObject {
  ...
  protected LayerObject(String category, Class<? extends AbstractStruct> classType, AbstractStruct parent) {
  ...
  /**
   * Returns the specific class type of the structure associated with the layer object for identification purposes.
   */
  ...
  /**
   * Returns the base structure the current structure belongs to. This can either be an {@code AreResource} or a
   * {@code WedResource} object.
   */
  ...
  /**
   * Returns the structure associated with the layer object. If the layer object consists of multiple structures, then
   * the first one available will be returned.
   *
   ...
   * multiple layer items (e.g. door polygons or ambient sounds/sound ranges).
   *
   * @return A list of layer items associated with the layer object. Never {@code null} and array do not contain
   *         {@code null}'s
   */
  public abstract AbstractLayerItem[] getLayerItems();

  /**
   * Updates the layer item positions. Takes zoom factor into account. Note: Always call this method after
   * loading/reloading structure data.
   */
   ...
   * @return {@code true} if the animation is active at the specified scheduled time, {@code false} otherwise.
   */
  ...
  protected static Polygon createPolygon(Point[] coords, double zoomFactor) {
    ...
    if (coords != null) {
      for (Point coord : coords) {
        poly.addPoint((int) (coord.x * zoomFactor + (zoomFactor / 2.0)),
            (int) (coord.y * zoomFactor + (zoomFactor / 2.0)));
      }
    }
    return poly;
  }
  ...
  /**
   * Converts the polygon from a global coordinate system into a local coordinate system where the top/left coordinate
   * is located at [0, 0].
   *
  ...
  protected static boolean isActiveAt(Flag flags, int dayTime) {
    if (flags != null && flags.getSize() > 2) {
      if (dayTime == ViewerConstants.LIGHTING_NIGHT) {
        // Night: 21:30..06:29
        return (flags.isFlagSet(0) || flags.isFlagSet(1) || flags.isFlagSet(2) || flags.isFlagSet(3)
            || flags.isFlagSet(4) || flags.isFlagSet(5) || flags.isFlagSet(21) || flags.isFlagSet(22)
            || flags.isFlagSet(23));
      } else {
        // Day: 06:30..21:29
        return (flags.isFlagSet(6) || flags.isFlagSet(7) || flags.isFlagSet(8) || flags.isFlagSet(9)
            || flags.isFlagSet(10) || flags.isFlagSet(11) || flags.isFlagSet(12) || flags.isFlagSet(13)
            || flags.isFlagSet(14) || flags.isFlagSet(15) || flags.isFlagSet(16) || flags.isFlagSet(17)
  ...
  protected static Image[] getIcons(Image[] defIcons) {
    final Image[] icons;
    final String keyIcon = SharedResourceCache.createKey(defIcons[0]) + SharedResourceCache.createKey(defIcons[1]);
  ...
  protected static void addTrappedDesc(StringBuilder sb, AbstractStruct struct, String trappedAttr,
      String difficultyAttr, String scriptAttr) {
