 ...
 * <p>
 * Polygon represents clickable area for interaction with door. As a rule, closed state geometry of door polygon matches
 * opened state geometry of the {@link LayerObjectDoor door itself} and vice versa.
 */
public class LayerObjectDoorPoly extends LayerObject {
  ...
  private void fillData(int start, int count, int offset, String[] info) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      final int vNum = ((IsNumeric) p.getAttribute(org.infinity.resource.wed.Polygon.WED_POLY_NUM_VERTICES, false))
          .getValue();
  ...
  private static org.infinity.resource.wed.Polygon getPolygonStructure(AbstractStruct baseStruct, int baseOfs,
      int index) {
    int idx = 0;
