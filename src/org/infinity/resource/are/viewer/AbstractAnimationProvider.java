...
public abstract class AbstractAnimationProvider implements BasicAnimationProvider {
  ...
  private boolean isActiveIgnored;
        ...
        while (ofs < maxOfs) {
          for (int x = 0; x < fw; x++) {
            ...
            if ((pixel & 0xff000000) != 0) {
              ...
              int g = (pixel >>> 8) & 0xff;
              int b = pixel & 0xff;
              r = (r * TilesetRenderer.LIGHTING_ADJUSTMENT[lighting][0]) >>> TilesetRenderer.LIGHTING_ADJUSTMENT_SHIFT;
              g = (g * TilesetRenderer.LIGHTING_ADJUSTMENT[lighting][1]) >>> TilesetRenderer.LIGHTING_ADJUSTMENT_SHIFT;
              b = (b * TilesetRenderer.LIGHTING_ADJUSTMENT[lighting][2]) >>> TilesetRenderer.LIGHTING_ADJUSTMENT_SHIFT;
              if (r > 255) {
                r = 255;
              }
              if (g > 255) {
