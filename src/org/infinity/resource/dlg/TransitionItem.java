...
class TransitionItem extends StateOwnerItem {
  ...
  /**
   * Item to which need go to in break cycles tree view mode. This item contains referense to the same transition as
   * this one (i.e. {@code this.trans == main.ref.trans})
   */
  private final DlgElement elem;

  /** Tree item to which go this transition or {@code null}, if this transition terminates dialog. */
  protected StateItem nextState;
  ...
  public ItemBase getMain() {
    return elem == null || elem.main == this ? null : elem.main;
  }
  ...
  public StateItem getChildAt(int childIndex) {
    return isMain() && childIndex == 0 ? nextState : null;
  }

  @Override
  public int getChildCount() {
    return isMain() && nextState != null ? 1 : 0;
  }

  @Override
  public TransitionOwnerItem getParent() {
    return parent;
  }

  @Override
  public int getIndex(TreeNode node) {
    return isMain() && node != null && node == nextState ? 0 : -1;
  }

  // Flag 3: Terminates dialogue
  @Override
  public boolean getAllowsChildren() {
    return isMain() && !trans.getFlag().isFlagSet(3);
  }
