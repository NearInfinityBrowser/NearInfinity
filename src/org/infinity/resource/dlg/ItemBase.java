...
abstract class ItemBase implements TreeNode {
  ...
  /**
   * Removes specified child item, returns {@code true} if child removed, {@code false} otherwize.
   *
  ...
  /**
   * Performs the given action for each child of the node until all childrens have been processed or the action throws
   * an exception. Exceptions thrown by the action are relayed to the caller.
   *
  ...
  protected final String getText(TreeItemEntry entry) {
    final String text = entry.hasAssociatedText() ? getText(entry.getAssociatedText()) : "(No text)";
