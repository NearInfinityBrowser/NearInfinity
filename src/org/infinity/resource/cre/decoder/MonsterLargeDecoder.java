...
/**
 * Creature animation decoder for processing type 9000 (monster_large) animations.
 * Available ranges: [9000,9fff]
 */
public class MonsterLargeDecoder extends SpriteDecoder
{
  /** The animation type associated with this class definition. */
  public static final AnimationType ANIMATION_TYPE = AnimationType.MONSTER_LARGE;

  private static final HashMap<Sequence, Couple<String, Integer>> suffixMap = new HashMap<Sequence, Couple<String, Integer>>() {{
    put(Sequence.STAND, Couple.with("G1", 0));
    put(Sequence.STANCE, Couple.with("G1", 8));
    put(Sequence.WALK, Couple.with("G1", 16));
    put(Sequence.ATTACK, Couple.with("G2", 0));
    put(Sequence.ATTACK_2, Couple.with("G2", 8));
    put(Sequence.ATTACK_3, Couple.with("G3", 0));
    put(Sequence.GET_HIT, Couple.with("G3", 8));
    put(Sequence.DIE, Couple.with("G3", 16));
    put(Sequence.SLEEP, get(Sequence.DIE));
    put(Sequence.GET_UP, Couple.with("!G3", 16));
    put(Sequence.TWITCH, Couple.with("G3", 24));
  }};
