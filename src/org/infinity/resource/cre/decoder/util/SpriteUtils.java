...
public class SpriteUtils {
  ...
  private static final HashMap<ResourceEntry, Couple<BamV1Decoder, BamV1Decoder.BamV1Control>> BAM_CACHE = new HashMap<>();
  /** Cache for replacement palettes. */
  private static final HashMap<ResourceEntry, int[]> PALETTE_CACHE = new HashMap<>();
  /** Cache for color gradients. */
  private static final HashMap<Integer, int[]> COLOR_GRADIENTS = new HashMap<>(350);
  /** Cache for randomized color gradients. */
  private static final HashMap<Integer, int[]> RANDOM_GRADIENT_INDICES = new HashMap<>();
  ...
  static {
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.EFFECT, EffectDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_QUADRANT, MonsterQuadrantDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_MULTI, MonsterMultiDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_MULTI_NEW, MonsterMultiNewDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_LAYERED_SPELL, MonsterLayeredSpellDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_ANKHEG, MonsterAnkhegDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.TOWN_STATIC, TownStaticDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.CHARACTER, CharacterDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.CHARACTER_OLD, CharacterOldDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER, MonsterDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_OLD, MonsterOldDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_LAYERED, MonsterLayeredDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_LARGE, MonsterLargeDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_LARGE_16, MonsterLarge16Decoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.AMBIENT_STATIC, AmbientStaticDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.AMBIENT, AmbientDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.FLYING, FlyingDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_ICEWIND, MonsterIcewindDecoder.class);
    TYPE_ASSOCIATION_MAP.put(AnimationInfo.Type.MONSTER_PLANESCAPE, MonsterPlanescapeDecoder.class);
  }
   ...
   * @param colors      Optional creature colors. Each entry uses the CRE_COLOR_xxx field names defined in
   *                    {@link CreResource} associated with the numeric color value.
   * @param equipment   Optional equipment. Each entry uses inventory slot index (as defined in CRE) as key and item
   *                    resref as value. These items are only used as source for overlay bams (i.e. weapons, shields,
   *                    helmets).
   ...
   * @return the {@code BamV1Decoder} instance created from the BAM resource. Returns {@code null} if decoder could not
   *         be retrieved.
   */
   ...
   *
   * @param entry the BAM resource entry.
   * @return the {@code BamV1Control} instance created from the BAM resource. Returns {@code null} if the controller
   ...
   *
   * @param entry the BAM resource entry.
   * @return {@code BamV1Decoder} and {@code BamV1Control} instances created from the BAM resource. Returns {@code null}
   *         if the objects could not be retrieved.
   */
  public static Couple<BamV1Decoder, BamV1Decoder.BamV1Control> loadBamDecoderController(ResourceEntry entry) {
    Couple<BamV1Decoder, BamV1Decoder.BamV1Control> retVal = BAM_CACHE.getOrDefault(entry, null);
    if (retVal == null) {
      try {
  ...
  /**
   * Loads the palette specified by the resource reference. It looks for a palette in the following order: [resref].BMP,
   * [resref].BAM.
   *
  ...
  /**
   * Loads the palette specified by the resource reference and index. It looks for a palette in the following order:
   * [resref+suffix].BMP, [resref+suffix].BAM, [resref].BMP, [resref].BAM.
   *
   * @param resref the resource reference to use for loading a palette.
  ...
  public static int[] loadReplacementPalette(String resref, int index) {
    ...
    retVal = PALETTE_CACHE.getOrDefault(entry, null);
    if (retVal == null) {
      try {
   ...
   * @return the updated palette.
   */
  public static int[] interpolateColors(int[] palette, int srcOfs1, int srcOfs2, int srcLen, int dstOfs, int dstLen,
      boolean allowAlpha) {
    int[] retVal = palette;
    if (palette != null && srcLen > 0 && dstLen > 0 && srcOfs1 >= 0 && srcOfs1 + srcLen <= palette.length
        && srcOfs2 >= 0 && srcOfs2 + srcLen <= palette.length && dstOfs >= 0 && dstOfs + dstLen <= palette.length) {
      retVal = Arrays.copyOf(palette, palette.length);
  ...
  /**
   * Returns the specified color gradient. Colors are retrieved from the game-specific gradient resource. Optionally
   * takes random color entry definitions into account.
   *
  ...
  public static int[] getColorGradient(int index, boolean allowRandom) {
    if (COLOR_GRADIENTS.isEmpty()) {
      ...
      if (palFile != null) {
        ...
        COLOR_GRADIENTS.put(Integer.MIN_VALUE, null);
      }
    }

    int[] retVal = allowRandom ? getRandomColorGradient(index) : null;
    if (retVal == null) {
      retVal = COLOR_GRADIENTS.getOrDefault(index, null);
    }
   ...
   * @return a randomly chosen color gradient from the random color entry list. Returns {@code null} if no real color
   *         gradient could be determined.
   */
  public static int[] getRandomColorGradient(int index) {
    if (RANDOM_GRADIENT_INDICES.isEmpty()) {
   ...
   * @return A rectangle object where x and y indicate the top-left corner relative to the center point. Width and
   *         height specify frame dimension.
   */
  ...
  /**
   * Determines the right allegiance color for selection circles and returns it as {@code Color} object. A negative
   * value will enable the "panic" color.
   *
  ...
  /**
   * Determines the right selection circle bitmap based on the specified allegiance value and returns it as
   * {@code Image} object. A negative value will enable the "panic" bitmap.
   *
   * @param value numeric allegiance value. Specify a negative value to override allegiance by the "panic" status.
   * @return
   */
  ...
  public static Class<? extends SpriteDecoder> getSpriteDecoderClass(AnimationInfo.Type type) {
    return TYPE_ASSOCIATION_MAP.get(type);
  }
   ...
   * @return a class type compatible with the specified animation id. Returns {@code null} if no class could be
   *         determined.
   */
  public static Class<? extends SpriteDecoder> getSpriteDecoderClass(int animationId) {
    ...
    if (animType != null) {
      retVal = TYPE_ASSOCIATION_MAP.get(animType);
    }
    ...
    if (retVal == null) {
      for (final AnimationInfo.Type type : AnimationInfo.Type.values()) {
        if (type.contains(animationId)) {
          retVal = TYPE_ASSOCIATION_MAP.get(type);
          if (retVal != null) {
            break;
          }
        }
  ...
  /**
   * Returns creature animation info in INI format. Section and field format is based on the EE v2.0 INI format. The
   * method will first look for existing INI data in the game resources. Failing that it will look up data in hardcoded
   * tables and fill in missing data from associated 2DA file if available. Failing that it will guess the correct
   * format based on animation type and available resources.
   *
   ...
   *
   * @return Class instance responsible for handling the detected animation type. {@code null} if type could not be
   *         determined.
   */
        ...
        if (Profile.getGame() == Profile.Game.PST || Profile.getGame() == Profile.Game.PSTEE) {
          ...
          for (final AnimationInfo.Type type : AnimationInfo.Type.values()) {
            if (type != AnimationInfo.Type.MONSTER_PLANESCAPE) {
              cls = TYPE_ASSOCIATION_MAP.get(type);
              if (isSpriteDecoderAvailable(cls, animationId, iniList)) {
                retVal = cls;
                break;
              }
  ...
  /**
   * A helper method that parses the specified data array and generates a list of INI lines related to the "general"
   * section.
   *
  ...
  public static List<String> processTableDataGeneral(String[] data, AnimationInfo.Type type) {
    ...
    if (animIndex < 0 || animIndex >= AnimationInfo.Type.values().length
        || AnimationInfo.Type.values()[animIndex] != type) {
      return retVal;
    }
  ...
  /**
   * A helper method for PST animations that parses the specified data array and generates a list of INI lines related
  ...
  /**
   * Returns whether the specified {@code SpriteDecoder} class is compatible with the given animation id and any of the
   * IniMap definitions.
   */
  private static boolean isSpriteDecoderAvailable(Class<? extends SpriteDecoder> spriteClass, int animationId,
      List<IniMap> iniList) {
    boolean retVal = false;
   ...
   * @return an list of {@link IniMap} instances with potential creature animation data. Returns {@code null} if no
   *         matching INI was found.
   */
   ...
   *
   * @param animationId the creature animation id
   * @return an list of {@link IniMap} instance with potential creature animation data. Returns empty list if no
   *         creature data was found.
   */
  ...
  /**
   * Returns creature animation info based on ANISND.2DA data and analyzing potential slot ranges. May return false
   * positives.
   *
   * @param animationId the creature animation id
   * @return a list of {@link IniMap} instances with potential creature animation data. Returns {@code null} if no
   *         potential match was found.
   */
  ...
  private static List<IniMap> guessIniMaps(int animationId) {
    ...
    if (resEntry != null) {
      ...
      if (anisrc != null) {
        ...
        if (iniSection != null) {
          for (IniMapEntry entry : iniSection) {
            try {
    ...
    switch (type) {
      ...
      case MONSTER_LAYERED_SPELL:
        ...
        if (bamEntry != null) {
          ...
          tableEntries
              .add(String.format("0x%04x %s 4 16 3 * %s * * * %d * * * * *", animationId, resref, palette, falseColor));
        }
        break;
      case MONSTER_ANKHEG:
      ...
      case TOWN_STATIC:
        ...
        if (bamEntry != null) {
          int falseColor = SpriteUtils.bamHasFalseColors(bamEntry) ? 1 : 0;
          tableEntries
              .add(String.format("0x%04x %s 7 16 3 * %s * * * %d * * * * *", animationId, resref, palette, falseColor));
        }
        break;
      case CHARACTER:
        ...
        if (bamEntry != null) {
          ...
          tableEntries
              .add(String.format("0x%04x %s 8 16 3 * * * * * %d %d 1 * * *", animationId, resref, falseColor, split));
        }
        break;
      case CHARACTER_OLD:
        bamEntry = ResourceFactory.getResourceEntry(resref + "1G1.BAM");
      ...
      case MONSTER:
        ...
        if (bamEntry != null) {
          ...
          int falseColor = SpriteUtils.bamHasFalseColors(bamEntry) ? 1 : 0;
          tableEntries.add(String.format("0x%04x %s 10 16 3 * %s * * * %d %d * * * *", animationId, resref, palette,
              falseColor, split));
          tableEntries.add(
              String.format("0x%04x %s 11 16 3 * %s * * * %d * * * * *", animationId, resref, palette, falseColor));
        }
        break;
      case MONSTER_OLD:
        resref = guessResourceRef(resref, "G1");
      ...
      case MONSTER_LARGE:
        ...
        if (bamEntry != null) {
          int falseColor = SpriteUtils.bamHasFalseColors(bamEntry) ? 1 : 0;
          tableEntries.add(
              String.format("0x%04x %s 12 16 3 * %s * * * %d * * * * *", animationId, resref, palette, falseColor));
        }
        break;
      case MONSTER_LARGE_16:
        ...
        if (bamEntry != null) {
          int falseColor = SpriteUtils.bamHasFalseColors(bamEntry) ? 1 : 0;
          tableEntries.add(
              String.format("0x%04x %s 13 16 3 * %s * * * %d * * * * *", animationId, resref, palette, falseColor));
        }
        break;
      case AMBIENT_STATIC:
        resref = guessResourceRef(resref, "G1");
        ...
        if (bamEntry != null) {
          int falseColor = SpriteUtils.bamHasFalseColors(bamEntry) ? 1 : 0;
          tableEntries.add(
              String.format("0x%04x %s 14 16 3 * %s * * * %d * * * * *", animationId, resref, palette, falseColor));
        }
        break;
      case AMBIENT:
        resref = guessResourceRef(resref, "G1");
        ...
        if (bamEntry != null) {
          int falseColor = SpriteUtils.bamHasFalseColors(bamEntry) ? 1 : 0;
          tableEntries.add(
              String.format("0x%04x %s 15 16 3 * %s * * * %d * * * * *", animationId, resref, palette, falseColor));
        }
        break;
      case FLYING:
        resref = guessResourceRef(resref, "");
        ...
        if (bamEntry != null) {
          int falseColor = SpriteUtils.bamHasFalseColors(bamEntry) ? 1 : 0;
          tableEntries.add(
              String.format("0x%04x %s 16 16 3 * %s * * * %d * * * * *", animationId, resref, palette, falseColor));
        }
        break;
