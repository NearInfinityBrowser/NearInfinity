...
/**
 * Creature animation decoder for processing type 2000 (monster_layered_spell) animations.
 * Note: This type can be incorrectly labeled as "monster_layered" in INI files
 * Available ranges: [2000,2fff]
 */
public class MonsterLayeredSpellDecoder extends SpriteDecoder
{
  /** The animation type associated with this class definition. */
  public static final AnimationType ANIMATION_TYPE = AnimationType.MONSTER_LAYERED_SPELL;

  public static final DecoderAttribute KEY_DUAL_ATTACK     = DecoderAttribute.with("dual_attack", DecoderAttribute.DataType.BOOLEAN);
  ...
  private static final HashMap<Sequence, Couple<String, Integer>> suffixMap = new HashMap<Sequence, Couple<String, Integer>>() {{
    ...
    put(Sequence.SPELL, Couple.with("G2", 8));
  ...
  public MonsterLayeredSpellDecoder(CreResource cre)
      throws Exception
  {
  ...
  /** ??? */
  public boolean isDualAttack() { return getAttribute(KEY_DUAL_ATTACK); }
  protected void setDualAttack(boolean b) { setAttribute(KEY_DUAL_ATTACK, b); }
...
//    }
    ...
    if (section.getEntryCount() == 0) {
      ...
      section = getAnimationInfo().getSection(AnimationType.MONSTER_LAYERED.getSectionName());
    }
