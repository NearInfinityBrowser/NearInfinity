...
/**
 * Creature animation decoder for processing type 1000 (multi_new) animations consisting of 4 segments. Available
 * ranges: [1300,13ff]
 */
public class MonsterMultiNewDecoder extends QuadrantsBaseDecoder {
  ...
  static {
    ...
    SUFFIX_MAP_SPLIT.put(Sequence.WALK, Couple.with("G1_1", 0));
    SUFFIX_MAP_SPLIT.put(Sequence.STANCE, Couple.with("G1_", 9));
    SUFFIX_MAP_SPLIT.put(Sequence.STAND, Couple.with("G1_2", 18));
    SUFFIX_MAP_SPLIT.put(Sequence.GET_HIT, Couple.with("G1_3", 27));
    SUFFIX_MAP_SPLIT.put(Sequence.DIE, Couple.with("G1_4", 36));
    SUFFIX_MAP_SPLIT.put(Sequence.SLEEP, SUFFIX_MAP_SPLIT.get(Sequence.DIE));
    SUFFIX_MAP_SPLIT.put(Sequence.GET_UP, Couple.with("!G1_4", 36));
    SUFFIX_MAP_SPLIT.put(Sequence.TWITCH, Couple.with("G1_5", 45));
    SUFFIX_MAP_SPLIT.put(Sequence.ATTACK, Couple.with("G2_", 0));
    SUFFIX_MAP_SPLIT.put(Sequence.ATTACK_2, Couple.with("G2_1", 9));
    SUFFIX_MAP_SPLIT.put(Sequence.ATTACK_3, Couple.with("G2_2", 18));
    SUFFIX_MAP_SPLIT.put(Sequence.ATTACK_4, Couple.with("G2_3", 27));
    SUFFIX_MAP_SPLIT.put(Sequence.ATTACK_5, Couple.with("G2_4", 36));
    SUFFIX_MAP_SPLIT.put(Sequence.SPELL, Couple.with("G2_5", 45));
    SUFFIX_MAP_SPLIT.put(Sequence.CAST, Couple.with("G2_6", 54));

    // Note: Replace underscore in suffix by one-based quadrant index
    SUFFIX_MAP_UNSPLIT.put(Sequence.WALK, Couple.with("G1_", 0));
    SUFFIX_MAP_UNSPLIT.put(Sequence.STANCE, Couple.with("G1_", 9));
    SUFFIX_MAP_UNSPLIT.put(Sequence.STAND, Couple.with("G1_", 18));
    SUFFIX_MAP_UNSPLIT.put(Sequence.GET_HIT, Couple.with("G1_", 27));
    SUFFIX_MAP_UNSPLIT.put(Sequence.DIE, Couple.with("G1_", 36));
    SUFFIX_MAP_UNSPLIT.put(Sequence.SLEEP, SUFFIX_MAP_UNSPLIT.get(Sequence.DIE));
    SUFFIX_MAP_UNSPLIT.put(Sequence.GET_UP, Couple.with("!G1_", 36));
    SUFFIX_MAP_UNSPLIT.put(Sequence.TWITCH, Couple.with("G1_", 45));
    SUFFIX_MAP_UNSPLIT.put(Sequence.ATTACK, Couple.with("G2_", 0));
    SUFFIX_MAP_UNSPLIT.put(Sequence.ATTACK_2, Couple.with("G2_", 9));
    SUFFIX_MAP_UNSPLIT.put(Sequence.ATTACK_3, Couple.with("G2_", 18));
    SUFFIX_MAP_UNSPLIT.put(Sequence.ATTACK_4, Couple.with("G2_", 27));
    SUFFIX_MAP_UNSPLIT.put(Sequence.ATTACK_5, Couple.with("G2_", 36));
    SUFFIX_MAP_UNSPLIT.put(Sequence.SPELL, Couple.with("G2_", 45));
    SUFFIX_MAP_UNSPLIT.put(Sequence.CAST, Couple.with("G2_", 54));
  }
  ...
  protected void init() throws Exception {
    ...
    Misc.requireCondition(getQuadrants() > 0 && getQuadrants() < 10, "Invalid number of quadrants: " + getQuadrants(),
  ...
  protected SeqDef getSequenceDefinition(Sequence seq) {
    ...
    if (getSuffixMap().containsKey(seq)) {
      ...
      int cycleOfs = getSuffixMap().get(seq).getValue1();
    ...
    if (!cycleList.isEmpty() && valid) {
      ...
      SeqDef tmp = SeqDef.createSequence(false, seq, dirs, true, cycleListE);
