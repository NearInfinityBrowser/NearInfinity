...
/**
 * Creature animation decoder for processing type 3000 (monster_ankheg) animations. Available ranges: [3000,3fff]
 */
public class MonsterAnkhegDecoder extends SpriteDecoder {
  ...
  static {
    ...
    SUFFIX_MAP.put(Sequence.DIE, Couple.with("G1", 1));
    SUFFIX_MAP.put(Sequence.SLEEP, SUFFIX_MAP.get(Sequence.DIE));
    SUFFIX_MAP.put(Sequence.GET_UP, Couple.with("!G1", 1));
    SUFFIX_MAP.put(Sequence.TWITCH, Couple.with("G1", 2));
    SUFFIX_MAP.put(Sequence.STAND_EMERGED, Couple.with("G1", 3));
    SUFFIX_MAP.put(Sequence.STAND_HIDDEN, Couple.with("G2", 0));
    SUFFIX_MAP.put(Sequence.EMERGE, Couple.with("G2", 1));
    SUFFIX_MAP.put(Sequence.HIDE, Couple.with("G2", 2));
    SUFFIX_MAP.put(Sequence.ATTACK, Couple.with("G3", 0));
    SUFFIX_MAP.put(Sequence.SPELL, Couple.with("G3", 1));
  }
  ...
  public boolean hasMirroredDirections() {
    return getAttribute(KEY_MIRROR);
  }

  protected void setMirroredDirections(boolean b) {
    setAttribute(KEY_MIRROR, b);
  }

  /** Returns whether the animation provides the full set of directions. */
  public boolean hasExtendedDirections() {
    return getAttribute(KEY_EXTEND_DIRECTION);
  }

  protected void setExtendedDirections(boolean b) {
