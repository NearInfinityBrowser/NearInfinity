...
/**
 * {@code ComboBoxModel} for item selection combo boxes used in the Creature Animation Browser. The model allows to
...
public class ItemSelectionModel extends AbstractListModel<ItemInfo> implements ComboBoxModel<ItemInfo> {
  ...
  public int getIndexOf(Object anItem) {
    if (anItem instanceof ItemInfo) {
      ...
      return IntStream.range(0, itemList.size()).filter(i -> entry.equals(itemList.get(i).getResourceEntry())).findAny()
          .orElse(-1);
    } else if (anItem != null) {
      final String itemName = anItem.toString().trim();
      return IntStream.range(0, itemList.size())
          .filter(i -> itemName.equalsIgnoreCase(itemList.get(i).getResourceEntry().getResourceRef())).findAny()
          .orElse(-1);
    }
    return -1;
  }
