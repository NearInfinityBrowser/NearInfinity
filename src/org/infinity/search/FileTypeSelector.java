...
/**
 * Widget that allow check resource types and get back list of selected resource pointers.
 *
 * @author Mingun
 */
public class FileTypeSelector extends JPanel implements ActionListener {
  /**
   * Stores last chosed array of selected checkboxes for each resource type. Each array has length equals to length of
   * {@link #filetypes}
   */
  ...
  /**
   * Creates panel with tho columns of checkboxex with abilities to check/unckeck all, invert current selection and
   ...
   * @param defaultStates Initial and default values for each file type. If length is less that length of
   *                      {@code filetypes}, tail values assumed to be {@code false}. If {@code null}, all values
  ...
  public FileTypeSelector(String title, String key, String[] filetypes, boolean[] defaultStates) {
    ...
    this.defaultStates = defaultStates.length < filetypes.length ? Arrays.copyOf(defaultStates, filetypes.length)
        : defaultStates;
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    } else if (src == bSet || src == bDefault && defaultStates == null) {
      for (final JCheckBox box : boxes) {
