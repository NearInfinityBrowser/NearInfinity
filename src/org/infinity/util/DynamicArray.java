...
/**
 * An array object which mimics to a certain degree the type casting behavior of pointers from other programming
 * languages. The byte order is initially set to little endian. Features include:
 * <ul>
...
public class DynamicArray {
   ...
   * @param offset  Buffer offset.
   * @param length  The number of bytes to convert.
   * @param charset The charset to be used to decode the bytes. Specify {@code null} to use "windows-1252" charset.
   * @return A string representation of the byte sequence or an empty string on error.
   */
  public static String getString(byte[] buffer, int offset, int length, Charset charset) {
    if (buffer != null && offset >= 0 && offset < buffer.length && length >= 0) {
      ...
      // XXX: Work around a bug in Java 6: String(buffer, ofs, len, charset) creates an internal copy of the whole
  ...
  public static int getUnsignedShort(byte[] buffer, int offset) {
    return (getShort(buffer, offset) & 0xffff);
  }
  ...
  public static long getUnsignedInt(byte[] buffer, int offset) {
    return (getInt(buffer, offset) & 0xffffffffL);
  }
  ...
  public int hashCode() {
    ...
    result = prime * result + Objects.hash(baseOfs);
    return result;
  }

  /**
   * Indicates whether this object is equal to another. The objects are equal if both point to the same array and are
   * set to the same base offset.
   *
   * @return {@code true} if both objects refer to the same array, starting at the same base offset.
  ...
  public boolean equals(Object obj) {
    ...
    return baseOfs == other.baseOfs && Arrays.equals(buffer, other.buffer);
  }
  ...
  /**
   * Sets a new base offset for the underlying array. All read/write operations take this offset into account
   * automatically.
   *
  ...
  public long getLong(int index) {
    ...
    if (ofs < buffer.length) {
      v |= (buffer[ofs++] & 0xffL) << 8;
    }
    if (ofs < buffer.length) {
      v |= (buffer[ofs++] & 0xffL) << 16;
    }
    if (ofs < buffer.length) {
      v |= (buffer[ofs++] & 0xffL) << 24;
    }
    if (ofs < buffer.length) {
      v |= (buffer[ofs++] & 0xffL) << 32;
    }
    if (ofs < buffer.length) {
      v |= (buffer[ofs++] & 0xffL) << 40;
    }
    if (ofs < buffer.length) {
      v |= (buffer[ofs++] & 0xffL) << 48;
    }
    if (ofs < buffer.length) {
      v |= (buffer[ofs++] & 0xffL) << 56;
    }
