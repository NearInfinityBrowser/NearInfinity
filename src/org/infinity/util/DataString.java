...
public class DataString<T> implements CharSequence, Comparable<CharSequence> {
   ...
   * @param s    the associated {@code String}.
   * @param data the associated data of type {@code T}.
   * @param fmt  a formatter string used by the {@link #toString()} method. Specifying {@code null} results in using the
   *             {@link #FMT_STRING_ONLY} formatter string.
   * @return a new {@code DataString} object.
   */
  ...
  /**
   * Returns the {@code char} value at the specified index. An index ranges from {@code 0} to {@code length() - 1}. The
   * first char value of the sequence is at index {@code 0}, the next at index {@code 1}, and so on, as for array
   * indexing.
   * <p>
   * The method is applied to the formatted string returned by {@link #toString()}.
   * <p>
   * If the {@code char} value specified by the index is a surrogate, the surrogatevalue is returned.
   */
  @Override
  ...
  /**
   * Returns a {@code CharSequence} that is a subsequence of this sequence. The subsequence starts with the {@code char}
   * value at the specified index and ends with the char value at index {@code end - 1}. The length (in {@code char}s)
   * of there turned sequence is {@code end - start}, so if {@code start == end} then an empty sequence is returned.
   * <p>
   * The method is applied to the formatted string returned by {@link #toString()}.
   *
  ...
  /**
   * Compares this object with the specified object for order. Returns anegative integer, zero, or a positive integer as
   * this object is lessthan, equal to, or greater than the specified object.
   * <p>
   * The method is applied to the formatted string returned by {@link #toString()}.
   */
  @Override
  ...
  public boolean equals(Object obj) {
    ...
    DataString<?> other = (DataString<?>) obj;
   ...
   * <p>
   * The formatter string may have up to two format specifiers. The first specifier is used for the string portion of
   * the {@code DataString}. The second specifier is used for the data portion of the {@code DataString}.
   * <p>
   * The position flag of the specifiers can be used to control order of output.
   *
   * @param fmt the formatter string. Specifying {@code null} results in using the {@link #FMT_STRING_ONLY} formatter
   *            string.
   */
