...
public class IdsMapCache {
  ...
  /**
   * A convenience function that returns the numeric value of the specified symbol. Returns a specified default value if
   * symbol could not be resolved (which can be {@code null}). <b>Special:</b> Returns {@code 0} if {@code symbol} is
   * "ANYONE" and {@code idsRef} matches one of the IDS resources used for IDS targeting.
   *
  ...
  /**
   * Makes sure that the specified IDS resource name is valid by checking filename length and adding extension if
   * needed.
   *
  ...
  public static boolean isCaseSensitiveMatch(String idsRef) {
    idsRef = getValidIdsRef(idsRef);
    if (idsRef != null) {
      return !idsRef.equalsIgnoreCase("TRIGGER.IDS") && !idsRef.equalsIgnoreCase("ACTION.IDS")
  ...
  /**
   * Returns a String array for Flag datatypes, that has been updated or overwritten with entries from the specified IDS
   * resource.
   *
   * @param flags     A static String array used as basis for Flag labels. {@code null} elements threat as unknown flags
