...
public class FindDataDialog extends JDialog implements ActionListener, ItemListener, DocumentListener {
  ...
  /**
   * Returns the unprocessed text string. Can be used as is for text type. Use {@link #getBytes()} to return the parsed
   * byte data.
   */
  ...
  /**
   * Returns the interpreted byte data from the search string if data type "Byte Values" is selected. Returns an empty
  ...
  private void init(Window parent) {
    ...
    gbc = ViewerUtil.setGBC(gbc, 0, 2, 1, 1, 0.0, 1.0, GridBagConstraints.FIRST_LINE_START, GridBagConstraints.BOTH,
    ...
    gbc = ViewerUtil.setGBC(gbc, 0, 3, 3, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_END, GridBagConstraints.HORIZONTAL,
        new Insets(8, 0, 0, 0), 0, 0);
    pMain.add(pButtons, gbc);

    gbc = ViewerUtil.setGBC(gbc, 0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.FIRST_LINE_END, GridBagConstraints.BOTH,
