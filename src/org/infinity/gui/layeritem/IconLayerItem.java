...
public class IconLayerItem extends AbstractLayerItem implements LayerItemListener {
   ...
   * @param show  {@code true} if the frame should be displayed for the specified visual state or {@code false}
   *              otherwise.
   */
  ...
  protected boolean isMouseOver(Point pt) {
    ...
    if (image != null) {
      Rectangle region = new Rectangle((getSize().width - image.getWidth() + rcCanvas.getX()) / 2,
          (getSize().height - image.getHeight() + rcCanvas.getY()) / 2, image.getWidth(), image.getHeight());
    ...
    if (state != null) {
      switch (state) {
        case NORMAL:
          ...
          break;
        case HIGHLIGHTED:
          label.setBackground(COLOR_BG_HIGHLIGHTED);
          break;
      }
  ...
  private static class FrameInfo {
    ...
    private static int getDefaultLineWidth() {
      return 1;
    }
