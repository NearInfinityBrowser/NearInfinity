 ...
 * the resource at which it starts and length in bytes in the resource.
 *
...
public abstract class Datatype implements StructEntry {
  ...
  public boolean equals(Object obj) {
    ...
    return length == other.length && Objects.equals(name, other.name) && offset == other.offset;
  }
  ...
  /**
   * Adds the specified update listener to receive update events from this object. If listener l is null, no exception
  ...
  /**
   * Removes the specified update listener, so that it no longer receives update events from this object.
   *
   * @param l The update listener
   */
  ...
  /**
   * If parent of this datatype is {@link AbstractStruct} then generates event that describe change in this object.
   * Property name in generated event is {@link #getName()} and owner is {@link #parent}.
   *
  ...
  void writeInt(OutputStream os, int value) throws IOException {
    ...
    switch (size) {
      ...
      case 1:
        ...
        break;
      default:
        throw new IllegalArgumentException("Field '" + name + "' of class " + getClass()
            + " has unsupported size; expected one of 1-4, but it has " + size);
    }
  }
